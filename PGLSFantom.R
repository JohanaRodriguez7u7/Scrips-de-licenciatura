...
tmp <- file.path(getwd(), paste("GenefamilySize.",trait,".associated.genes.",Correcting,".correction.log", sep = ""))
lf <- log_open(tmp, traceback = F)

#setwd("/home/jvar2023/Bases")

setwd("C:/Users/Johana Banana/Documents/fantom/Modifiaciones/PGLS")
...
#*************   Change parameters!!!!!!!   ********###
list_orthogroups_counts<-read.csv("Normalizacion_Fantom_CuartilesVERDADERO.csv",row.names = 1)
traits<-read.csv("Long_days_Fantom_exacto.csv")
tree<-read.tree("DendrogramaCuartiles.nwk")
#*************   ************************** **
#*************   ************************** ********###
...
#Remove Species without phenotypic or sequencing data #*************   Change parameters!!!!####
traits <- traits[!is.na(traits$Estimated_longevity_days_to_normal),]

## removing species from tree to match traits file
#*************   Change parameters!!!!!!!   ********###
tree2<-drop.tip(tree,setdiff(tree$tip.label, traits$fantom.data))
#*************   ************************** ********###
...
#*************   Change parameters!!!!!!!   ********###
Trait.orthoGr.filtering(traits = traits, pheno.col.nums = 3, spp.col.num = 4, gene.numbers= gene_numbers)
#Trait.orthoGr.filtering(traits = traits, pheno.col.nums = 3, spp.col.num = 4,gene.numbers = gene.numbers.filtered)
#*************   ************************** ********###
#gene.numbers.filtered <- gene_numbers
write.csv(gene.numbers.filtered, paste("Filtered.GeneNum.",dim(gene.numbers.filtered)[2],"spp",trait,".csv", sep = ""))
...
#gene.numbers.filtered <- gene.numbers.filtered[traits.filtered[,1]]
gene.numbers.filtered <- gene.numbers.filtered[traits.filtered$fantom.data]
#*************   ************************** ********###
dim(traits.filtered)
# colnames(gene.numbers.filtered)
# traits.filtered$Species.biomart
...
PGLS.GF.size<- function(traits, pheno.col.nums, tree, spp.col.num, gene.numbers, No.variables, where.Save.it){
  ...
  if (No.variables == 1){
    ...
    out<<-as.data.frame(t(apply(X = gene.numbers,1,FUN = function(x){
      ...
      tree<- drop.tip(tree2,names(x)[is.na(x)]) 
...
dirSave<-"C:/Users/Johana Banana/Documents/fantom/Modifiaciones/PGLS"
#*************   ************************** ********###
...
#*************   Change parameters!!!!!!!   ********###
PGLS.GF.size(traits = traits.filtered, pheno.col.nums = c(3), No.variables = 1, 
             tree = tree2, spp.col.num = 4, gene.numbers = gene.numbers.filtered, 
...
#*************   Change parameters!!!!!!!   ********###
PGLS.GF.size.perms(traits.cl = traits.filtered, cores = 20, pheno.col.nums = 3, tree.cl = tree2, spp.col.num = 4, 
...
#out<-read.csv("PGLS_perms.Neuron_density.18.Spp.2777000.Perm_GeneFams.Rs_benjamini.2023-05-17.csv", row.names = 1)
...
DensityPlots(out = out, outcol = "R t value Estimated_longevity_days_to_normal", perms = perms, permscol= "R t value Estimated_longevity_days_to_normal", 
             traits.cl = traits.filtered, spp.col.num = 1, pheno.col.nums = 52, 
